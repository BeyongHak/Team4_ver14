...
def generate_full_policy_map(env, model, device, config_name, save_path, is_discrete):
    ...
    # 2. Define State Coverage
    limit_val = 10
    limits = [limit_val] * len(env.retailers)
    total_combs = limit_val ** len(env.retailers)
    
    # Check feasibility
    if total_combs > 500000:
...
class SAAPolicy:
    def __init__(self, config_name, num_retailers, is_discrete, save_path=None):
        ...
        csv_path = os.path.join("policy_SAA", group_id, "SAA_policy_map.csv")
        
        if not os.path.exists(csv_path):
             alt_path = os.path.join("result_SAA", config_name, "SAA_policy_map.csv")
             if os.path.exists(alt_path):
                 csv_path = alt_path
                 print(f"  [SAA] Note: Using legacy path {alt_path}")

        if not os.path.exists(csv_path):
            raise FileNotFoundError(f"SAA Policy Map not found at {csv_path}. Run SAA_LeadtimeDemand.py first.")
        ...
        with open(csv_path, 'r') as f:
            ...
            for row in reader:
                try:
                    ...
                    # Track Max Index
                    current_max = max(key)
                    if current_max > max_idx: max_idx = current_max
                except: pass
        
        # 2. Create Dense Arrays
        # Shape: (Max+1, Max+1, ..., Max+1) for NumRetailers
        # Limit max dimension to avoid explosion if keys are huge (e.g. >100)
        # Assuming compact states usually < 50.
        self.dim_size = max_idx + 1
        shape = tuple([self.dim_size] * num_retailers)
        
        print(f"  [SAA] Creating Dense Policy Table: Shape {shape} ({self.dim_size}^ {num_retailers})")
        ...
        for k, v in raw_map.items():
            if all(x < self.dim_size for x in k):
    ...
    def get_action(self, obs, info):
        ...
        for x in key:
            ix = int(x)
            if ix >= self.dim_size: ix = self.dim_size - 1
            if ix < 0: ix = 0
            final_key.append(ix)
...
def make_env(config, seed, is_discrete):
    def _thunk():
        ...
        for r_conf in config['retailers']:
            ...
            retailer_objects.append(Retailer(delta=delta, customer=customer_dist))
...
def run_numerical_experiment(args):
    ...
    if args.mode == "discrete":
        ...
        result_base_dir = f"result_D_sym"
    elif args.mode == "continuous":
        model_base_dir = f"saved_model_C"
        result_base_dir = f"result_C_sym"
    else:
    ...
    if not os.path.exists(summary_path):
        with open(summary_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["Config", "LeadTime", "NumRetailers", "Heterogeneity", "Decoding", "Avg_Cost_Per_Step", "CI_95_Lower", "CI_95_Upper"])
    ...
    for idx_exp, exp in enumerate(experiments):
        ...
        try:
            with open(summary_path, 'a', newline='') as f:
                writer = csv.writer(f)
                decoding_str = "Residual" if args.policy != 'saa' else "N/A"
                writer.writerow([config_name, lead_time, real_num_retailers, hetero_type, decoding_str, f"{mean_cost:.2f}", f"{ci_lower:.2f}", f"{ci_upper:.2f}"])
        except Exception as e:
            print(f"  [Error] Failed to write to summary.csv: {e}")
            
        # Policy Map (DISABLED for Symmetric Eval)
        # if args.policy != 'saa':
        #     try:
